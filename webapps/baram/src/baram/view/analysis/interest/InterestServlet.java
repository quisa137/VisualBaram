package baram.view.analysis.interest;
...
public class InterestServlet extends HttpServlet {
	...
	Logger logger = Logger.getLogger(InterestServlet.class.getName());
	...
	protected void service(HttpServletRequest request,
		...
		if (!User.checkSession(request, logger,
			...
			session.setAttribute("ref", "./Interest");
			out.println("<script>");
		...
		out.println(CommonHtml.getHtml("side", new String[] {
				"<div class='sideNoClickAction' id='side3'></div>",
				"<div class='sideClickAction'></div>", "img/side/s3.png",
				"img/side/s3_a.png" }));
		...
		try {
			...
			int count=rs.getInt("count(*)");
				...
				sb.append("{k : '"+ errCode + "', level : '" + logIP +"', v : "+count+ "}");
		...
		out.println("<a class='ContentsTitle'>Bar Chart</a>");
//		out.println("<a class='popupBtn' id='btnClick'>Quickstart Guide</a>");
	...
	out.println(sb.toString());
		...
		out.println("<div class='dc-chart' id='main-chart'  font: 12px sans-serif; color: #6d6d73;'></div>");
		...
		out.println("<script type='text/javascript' src='js/Interest.js'></script>");
