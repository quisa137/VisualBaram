package baram.view.analysis.population;
...
public class PopulationServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (!User.checkSession(request, logger, "GetAllMemberInfoServlet.service", this)) {
			session.setAttribute("ref", "./Population");
			out.println("<script>");
		...
		out.println(CommonHtml.getHtml("head",
				new String[] { "<title>Analysis Demo</title>", "<title>HeatMap Chart</title>" }));
		...
		out.println(CommonHtml.getHtml("side", new String[] { "<div class='sideNoClickAction' id='side6'></div>",
				"<div class='sideClickAction'></div>", "img/side/s6.png", "img/side/s6_a.png" }));
		...
		out.println("<a class='ContentsTitle'>HeatMap Chart</a>");
//		out.println("<a class='popupBtn' id='btnClick'>Quickstart Guide</a>");
		...
		out.println(
				"<div id='heatMapChart' style='margin-left:15px'></div>");

		out.println("</div>");

		out.println("<script type='text/javascript' src='js/Population.js'></script>");
		out.println("</div>");
		out.println("</div>");
		out.println("</div>");
