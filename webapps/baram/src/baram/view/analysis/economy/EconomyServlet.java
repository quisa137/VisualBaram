package baram.view.analysis.economy;
...
public class EconomyServlet extends HttpServlet {
	...
	Logger logger = Logger.getLogger(EconomyServlet.class.getName());
	...
	protected void service(HttpServletRequest request,
		...
		if (!User.checkSession(request, logger,
			...
			session.setAttribute("ref", "./Economy");
			out.println("<script>");
		...
		out.println(CommonHtml.getHtml("side", new String[] {
				"<div class='sideNoClickAction' id='side10'></div>",
				"<div class='sideClickAction'></div>", "img/side/s10.png",
				"img/side/s10_a.png" }));
		...
		try {
			...
			rs = stmt.executeQuery("select c6, c13 , c14 from "+MainServlet.dbmsOutputTable+" group by c6;");
			...
			while (rs.next()) {
				...
				sb.append("{Run : '" + c6 + "', Speed : "+c13+ "}, {Run : '" + c6 + "', Speed : "+c14+"}");
		...
		out.println("<a class='ContentsTitle'>Boxplot Chart</a>");
//		out.println("<a class='popupBtn' id='btnClick'>Quickstart Guide</a>");
	...
	out.println(sb.toString());
		...
		out.println("<div id='box-test'></div>");
		...
		out.println("<script type='text/javascript' src='js/Economy.js'></script>");
		out.println("</div>");
		out.println("</div>");
		out.println("</div>");
