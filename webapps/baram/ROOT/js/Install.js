...
gameRingChart.width(chartWidth * 0.5).height(chartHeight).dimension(gameDim)
		.group(installPerGame).innerRadius(Math.round(chartHeight * 0.15)).ordinalColors(chartColors);

installHistChart.width(chartWidth * 0.9).height(chartHeight).margins({
	top : 0,
	right : 20,
	bottom : 80,
	left : 20
...
installerRowChart.width(chartWidth * 0.4).height(chartHeight).dimension(osDim)
		.group(installPerOs).elasticX(true).ordinalColors(chartColors);
